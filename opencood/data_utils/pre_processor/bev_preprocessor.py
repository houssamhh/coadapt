...
class BevPreprocessor(BasePreprocessor):
    ...
    def preprocess(self, pcd_raw):
        ...
        #     # Clamp to BEV tensor bounds
        #     x = min(max(x, 0), bev.shape[0] - 1)
        #     y = min(max(y, 0), bev.shape[1] - 1)
        #     z = min(max(z, 0), bev.shape[2] - 1)
        ...
        # indices should be (N, 3) integer array
        indices = indices.astype(np.int64, copy=False)

        # keep only indices inside bounds
        valid = (
            (indices[:, 0] >= 0) & (indices[:, 0] < bev.shape[0]) &
            (indices[:, 1] >= 0) & (indices[:, 1] < bev.shape[1]) &
            (indices[:, 2] >= 0) & (indices[:, 2] < bev.shape[2])
        )

        idx = indices[valid]
        bev[idx[:, 0], idx[:, 1], idx[:, 2]] = 1
