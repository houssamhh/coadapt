...
try:
    ...
    def bbox_overlaps(boxes, query_boxes):
        """Pure-NumPy fallback for bbox_overlaps (used when Cython ext is absent)."""
        b = boxes[:, None, :]        # (N, 1, 4)
        q = query_boxes[None, :, :]  # (1, K, 4)
        iw = _np.minimum(b[:, :, 2], q[:, :, 2]) - _np.maximum(b[:, :, 0], q[:, :, 0]) + 1
        ih = _np.minimum(b[:, :, 3], q[:, :, 3]) - _np.maximum(b[:, :, 1], q[:, :, 1]) + 1
        iw = _np.maximum(iw, 0)
        ih = _np.maximum(ih, 0)
        inter = iw * ih
        box_area = (b[:, :, 2] - b[:, :, 0] + 1) * (b[:, :, 3] - b[:, :, 1] + 1)
        query_area = (q[:, :, 2] - q[:, :, 0] + 1) * (q[:, :, 3] - q[:, :, 1] + 1)
        union = box_area + query_area - inter
        return (inter / _np.where(union > 0, union, 1e-10)).astype(_np.float32)
