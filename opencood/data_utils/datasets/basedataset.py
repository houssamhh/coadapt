...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        scenario_folders = sorted([os.path.join(root_dir, x)
                                   ...
                                   os.path.isdir(os.path.join(root_dir, x))]) # correct
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            # be ego vehicle.
            # Guard: folder names like 'cav_208' (MultiV2X) are not numeric.
            try:
                if int(cav_list[0]) < 0:
                    cav_list = cav_list[1:] + [cav_list[0]]
            except ValueError:
                pass  # non-numeric names (e.g. 'cav_208') — no reordering needed
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                cav_path = os.path.join(scenario_folder, cav_id) # correct

                # use the frame number as key, the full path as the values [CORRECT]
                yaml_files = \
    ...
    def _resolve_path(self, p: str) -> str:
        """
        Make dataset file paths robust:
        - accepts absolute paths
        - accepts relative paths from validate_dir
        - avoids double-prefixing validate_dir
        """
        if p is None:
            return p
        ...
        # You need the root directory used by your dataset.
        # In OpenCOOD, this is typically self.params['validate_dir'] (for test)
        # or something like self.root_dir / self.data_path depending on the dataset.
        root = self.params.get('validate_dir', None) or self.params.get('data_dir', None)
        if root is None:
            # fallback: don't change path if we can't resolve root
            return p_norm
        ...
        # If path already contains the root, don't prefix again
        # (this handles your duplicated case)
        if root_norm in p_norm:
            return p_norm

        # Otherwise, join with root
        return os.path.normpath(os.path.join(root_norm, p_norm))
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            # Skip entries with no usable pose (e.g. RSUs without lidar_pose /
            # true_ego_pose — their distance was set to inf by calc_dist_to_ego)
            if cav_content.get('distance_to_ego') == float('inf'):
                continue
            ...
            # skip if either this CAV or the ego is missing pose at these timestamps
            def _yaml_pose(content, tkey):
                if tkey not in content:
                    return None
                d = load_yaml(content[tkey]['yaml'])
                return d.get('lidar_pose') or d.get('true_ego_pose')

            ego_poses_missing = (
                _yaml_pose(ego_cav_content, timestamp_key) is None
                or _yaml_pose(ego_cav_content, timestamp_key_delay) is None
            )
            if ego_poses_missing:
                # Ego pose unavailable — cannot process any CAV for this frame
                return OrderedDict()
            if (_yaml_pose(cav_content, timestamp_key) is None
                    or _yaml_pose(cav_content, timestamp_key_delay) is None):
                del data[cav_id]
                continue
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        def _get_pose(yaml_dict):
            """Return lidar pose; fall back to true_ego_pose for MultiV2X."""
            return (yaml_dict.get('lidar_pose')
                    or yaml_dict.get('true_ego_pose'))
        ...
        for cav_id, cav_content in scenario_database.items():
            if cav_content['ego']:
                ...
                if timestamp_key in cav_content:
                    ego_lidar_pose = _get_pose(
                        load_yaml(cav_content[timestamp_key]['yaml']))
                break

        if ego_lidar_pose is None:
            # Ego pose unavailable for this timestamp — skip distance calculation
            for cav_id, cav_content in scenario_database.items():
                cav_content['distance_to_ego'] = 0.0 if cav_content.get('ego') else float('inf')
        ...
        for cav_id, cav_content in scenario_database.items():
            if timestamp_key not in cav_content:
                # This CAV/RSU does not have data for this timestamp
                cav_content['distance_to_ego'] = float('inf')
                scenario_database.update({cav_id: cav_content})
                continue
            cur_lidar_pose = _get_pose(
                load_yaml(cav_content[timestamp_key]['yaml']))
            if cur_lidar_pose is None:
                # No pose available (e.g. RSU without lidar_pose/true_ego_pose)
                # Assign a large distance so this agent is excluded by range filter
                cav_content['distance_to_ego'] = float('inf')
                scenario_database.update({cav_id: cav_content})
                continue
            distance = \
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        delay_cav_lidar_pose = _pose(delay_params)
        delay_ego_lidar_pose = _pose(delay_ego_params)

        cur_ego_lidar_pose = _pose(cur_ego_params)
        cur_cav_lidar_pose = _pose(cur_params)
        ...
        # 'vehicles' is the OPV2V key; MultiV2X and other datasets may use
        # 'objects' or another key — try in order and fall back to empty dict.
        gt_key = next(
            (k for k in ('vehicles', 'objects', 'annotations', 'labels')
             if k in cur_params),
            None
        )
        delay_params['vehicles'] = cur_params[gt_key] if gt_key is not None else {}
