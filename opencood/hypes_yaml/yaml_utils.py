...
def load_yaml(file, opt=None):
    ...
    if isinstance(file, str):
        ...
        norm = file.replace("\\", "/")
        parts = norm.split("/")
        # heuristic: if the sequence "<town>/<date>/<cav_id>" appears twice, keep from the second
        # (works for your exact failure)
        if len(parts) >= 6:
            tail = "/".join(parts[-6:])  # adjust if your layout differs
            # if tail occurs twice, keep last
            if norm.count(tail) >= 2:
                norm = norm[norm.rfind(tail):]
        file = norm.replace("/", os.sep)
